...
export default function PublicPolicyDetailPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        // Fetch policy via API so timestamps are plain objects
        ...
        // Load critique + simulation histories (client SDK is fine)
        ...
        setCritiques(critSnap.docs.map((d) => ({ id: d.id, ...(d.data() as any) })));
        setSimulations(simSnap.docs.map((d) => ({ id: d.id, ...(d.data() as any) })));
  ...
  const handleDownload = async () => {
    ...
    // Prefer publicPdfUrl if available
        ...
        <div className="flex items-start justify-between gap-4 mb-6">
          ...
          {!loading && policy && (
            <div className="flex gap-2 flex-wrap">
              ...
              <Button
                onClick={() =>
                  router.push(`/login?next=${encodeURIComponent(`/critique?policyId=${policy.id}`)}`)
                }
              >
                Log in to Critique
              </Button>
              ...
              <Button
                variant="outline"
                onClick={() =>
                  router.push(`/login?next=${encodeURIComponent(`/simulations?policyId=${policy.id}`)}`)
                }
              >
                Log in to Simulate
              </Button>
        ...
        <Card className="p-4 mb-6 border-blue-electric/30 bg-blue-soft">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <p className="text-sm text-blue-deep">
              To run AI Critique, Simulations, or upload policies, please log in or create an account.
            </p>
            <div className="flex gap-2 flex-wrap">
        ...
        {policy && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <Card className="p-6">
                <div className="flex items-center gap-2 flex-wrap mb-3">
                  ...
                  {policy.sector && <Badge variant="outline">{policy.sector}</Badge>}
                </div>
                ...
                {(policy.updatedAtMs || policy.createdAtMs) && (
                  <p className="text-xs text-[var(--text-secondary)] mt-4">
                    Updated: {formatWhenMs(policy.updatedAtMs)}{" "}
                    {policy.createdAtMs ? ` • Created: ${formatWhenMs(policy.createdAtMs)}` : ""}
                  </p>
                )}
              </Card>
                          ...
                          >
                            <div className="text-left">
                              ...
                              <p className="text-xs text-[var(--text-secondary)] mt-1">
                                ...
                                {typeof c.revisionNumber === "number" ? ` • Revision ${c.revisionNumber}` : ""}
                              </p>
                            </div>
                            {isOpen ? (
                ...
                {simulations.length === 0 ? (
                  ...
                  <div className="space-y-3 mt-4">
                    {simulations.map((s) => {
                      ...
                      const outputs = s?.outputs ?? s?.results ?? {};
                      ...
                      return (
                        <div key={s.id} className="border rounded-xl overflow-hidden">
                          ...
                          >
                            <div className="text-left">
                              <p className="font-bold text-blue-deep">
                                Scenario: {inputs?.scenarioName ?? "Simulation"}
                              </p>
                              <p className="text-xs text-[var(--text-secondary)] mt-1">
                          ...
                          {isOpen && (
                            <div className="px-4 pb-4">
                              ...
                              <div className="mt-4">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                                  {Object.entries(outputs).slice(0, 8).map(([k, v]) => (
                                    <div key={k} className="border rounded-lg px-3 py-2">
                                      <span className="font-semibold text-blue-deep">{k}:</span>{" "}
                                      <span className="text-[var(--text-secondary)]">
                                        {typeof v === "object" ? JSON.stringify(v) : String(v)}
            ...
            <div className="space-y-6">
              ...
              <Card className="p-6">
                <h2 className="text-lg font-bold text-blue-deep mb-3">Downloads</h2>
                ...
                {(policy.publicPdfUrl || policy.storagePath) ? (
                  ...
                  <p className="text-sm text-[var(--text-secondary)]">
                    Download is not available for this policy.
                  </p>
                )}
              </Card>

              <Card className="p-6">
                <h2 className="text-lg font-bold text-blue-deep mb-3">Actions</h2>
                <p className="text-sm text-[var(--text-secondary)] mb-4">
                  Log in to run AI Critique or Simulations.
                </p>
