...
export default function CritiquePage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // pull previous critique score for this policy (latest)
  ...
  await saveCritique({
    ...
    // no previous score for brand new AI policy
  ...
  // ✅ Only original uploader can upload revised version
  const isOwner = useMemo(() => {
    if (!user || !policy) return false;
    // createdByUid exists on uploaded & ai_generated policies
  ...
  try {
    ...
    // 1) Create the AI policy in repository + storage PDF
    ...
    // 2) Auto-critique the generated policy immediately (same criteria)
    const aiScore = await autoCritiqueGeneratedPolicy({
      aiPolicyId: created.id,       // make sure createAIGeneratedPolicy returns id
    ...
    // 3) Redirect to AI policy details
  ...
  const run = async (opts?: { forcePreviousScore?: number | null }) => {
    ...
    try {
      ...
      setResult(out);
      setPreviousScore(out.overallScore); // next run compares against this
  ...
  const handleRevisedFile = async (file: File) => {
    ...
    try {
      ...
      // Upload revised file to Storage
      ...
      // Update existing policy doc ONLY (no new policy)
      ...
      // Update local state so next critique uses new contentText
      const prevScoreSnapshot = previousScore; // compare improvement against last
      setPolicy((p) =>
        p
          ? ({
              ...p,
      ...
      // Re-run critique with same criteria and improvement check
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card className="p-6">
              ...
              <Button
                ...
                onClick={() => router.push("/policies/upload?redirect=/critique")}
              >
                <Upload size={16} />
                Upload policy
              </Button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="p-6 lg:col-span-2">
              ...
              <div className="flex items-center gap-2 mb-3 flex-wrap">
                ...
                {overallTone === "mid" && (
                  <span className="text-sm text-amber-700 font-semibold">Moderate</span>
                )}
                {overallTone === "good" && (
            ...
            <Card className="p-6">
              ...
              <div className="space-y-2">
                ...
                {/* ✅ Only show revised upload button if owner */}
                {isOwner ? (
                  <Button
                    variant="outline"
                    className="w-full gap-2"
